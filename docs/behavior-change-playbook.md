...
- Tenant owns tenant-scoped channel connection metadata and encrypted provider configuration. Direct and Broadcast store only the `ChannelConnectionId` they use; do not add cross-database EF navigations or duplicate provider secrets in product databases.
- `Tenant.Id` is the canonical tenant/workspace boundary across Base, Direct, Broadcast, and shared control-plane behavior. Pass the selected tenant ID unchanged to module persistence and Channel Connection behavior; do not introduce a second grouping identifier or resolve sibling tenant rows. The module selects the physical database through its current `TenantConnection`, not through a different tenant ID.
