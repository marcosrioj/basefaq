...
SELECT 'CREATE DATABASE qf_direct_db_01'
    WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'qf_direct_db_01')\gexec

SELECT 'CREATE DATABASE qf_broadcast_db_01'
    WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'qf_broadcast_db_01')\gexec
