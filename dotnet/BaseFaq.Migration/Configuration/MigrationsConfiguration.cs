...
namespace BaseFaq.Migration.Configuration;
...
internal static class MigrationsConfiguration
{
    public static IConfiguration Build(string? solutionRoot = null)
    {
        ...
        {
            builder.AddJsonFile(
                Path.Combine(solutionRoot, "dotnet", "BaseFaq.Tenant.Portal.Api", "appsettings.json"),
                optional: true);

            if (!string.IsNullOrWhiteSpace(environmentName))
            {
                builder.AddJsonFile(
                    Path.Combine(solutionRoot, "dotnet", "BaseFaq.Tenant.Portal.Api",
