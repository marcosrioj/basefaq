...
namespace BaseFaq.Faq.Common.Persistence.Seed.Application;
...
public sealed class FaqSeedService : IFaqSeedService
{
    ...
    private static List<FaqDb.Entities.Faq> BuildFaqs(
        int count,
        Guid tenantId,
        FaqStatus[] statuses,
        FaqSortStrategy[] sortStrategies)
    {
        ...
        var faqs = new List<FaqDb.Entities.Faq>(count);
        ...
        for (var i = 1; i <= count; i++)
        {
            ...
            faqs.Add(new FaqDb.Entities.Faq
            {
                Id = Guid.NewGuid(),
                TenantId = tenantId,
                Name = $"FAQ {i:000}",
