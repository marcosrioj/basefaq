...
public sealed class ChannelConnectionsGetListQueryHandler(TenantDbContext dbContext)
    : IRequestHandler<ChannelConnectionsGetListQuery, PagedResultDto<ChannelConnectionDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (dto.TenantId.HasValue)
            query = query.Where(connection => connection.TenantId == dto.TenantId.Value);
    ...
    private static IQueryable<ChannelConnectionEntity> ApplySorting(
        ...
        string? sorting) => sorting?.Trim().ToLowerInvariant() switch
        {
            ...
            "tenantid" or "tenantid asc" => query.OrderBy(connection => connection.TenantId),
            "tenantid desc" => query.OrderByDescending(connection => connection.TenantId),
            "status" or "status asc" => query.OrderBy(connection => connection.Status),
            "status desc" => query.OrderByDescending(connection => connection.Status),
