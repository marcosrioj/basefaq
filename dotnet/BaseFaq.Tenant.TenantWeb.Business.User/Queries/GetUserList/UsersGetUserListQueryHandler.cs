...
namespace BaseFaq.Tenant.TenantWeb.Business.User.Queries.GetUserList;
    ...
    : IRequestHandler<UsersGetUserListQuery, PagedResultDto<UserDto>>
{
    ...
    private static IQueryable<BaseFaq.Common.EntityFramework.Tenant.Entities.User> ApplySorting(
        IQueryable<BaseFaq.Common.EntityFramework.Tenant.Entities.User> query, string? sorting)
    {
        if (string.IsNullOrWhiteSpace(sorting))
        {
            return query.OrderBy(user => user.GivenName);
        }

        IOrderedQueryable<BaseFaq.Common.EntityFramework.Tenant.Entities.User>? orderedQuery = null;
        ...
        return orderedQuery ?? query.OrderBy(user => user.GivenName);
    }

    private static IOrderedQueryable<BaseFaq.Common.EntityFramework.Tenant.Entities.User> ApplyOrder(
        IQueryable<BaseFaq.Common.EntityFramework.Tenant.Entities.User> query,
        string fieldName,
        bool desc,
        bool isFirst)
    {
        return fieldName.ToLowerInvariant() switch
        {
            ...
            "id" => isFirst
                ...
                : (desc
                    ...
                    .ThenBy(user => user.Id)),
            _ => isFirst
                ? query.OrderBy(user => user.GivenName)
                : ((IOrderedQueryable<BaseFaq.Common.EntityFramework.Tenant.Entities.User>)query)
                .ThenBy(user => user.GivenName)
        };
    }
