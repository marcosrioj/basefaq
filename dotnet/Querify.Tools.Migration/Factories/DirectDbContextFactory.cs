...
public sealed class DirectDbContextFactory : IDesignTimeDbContextFactory<DirectDbContext>
{
    public DirectDbContext CreateDbContext(string[] args)
    {
        ...
        if (module != ModuleEnum.Direct)
        {
            throw new InvalidOperationException(
                $"Module '{module}' is not supported by {nameof(DirectDbContextFactory)}.");
        }
        ...
        {
            throw new InvalidOperationException(
                "Failed to connect to the tenant database while creating the Direct DbContext. " +
                "Make sure the database is running and ConnectionStrings:TenantDb is set correctly, " +
                "or provide ConnectionStrings:DirectDb for offline design-time scaffolding.",
                ex);
        }
    ...
    {
        var directDbConnectionString = configuration.GetConnectionString("DirectDb");
        if (!string.IsNullOrWhiteSpace(directDbConnectionString))
        {
            return directDbConnectionString;
        }
    ...
    private static TenantConnection ResolveCurrentConnection(TenantDbContext tenantDbContext)
    {
        return tenantDbContext
            .GetCurrentTenantConnection(ModuleEnum.Direct)
            .GetAwaiter()
            .GetResult();
    }
